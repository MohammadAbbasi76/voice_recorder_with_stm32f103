...
void init_value()
{
  ...
  SevenSegmentDisplay(WitchVoiceWannaToPlay);
}
void FlashEraseFunc()
{
  if (!W25qxx_Init())
...
void SetupForPlay(uint8_t VoiceNumber)
{
  if (voice.WitchVoiceIsRecord[VoiceNumber] == 1)
  {
    ...
    RestoreDetail(voice.WitchVoiceIsRecord, voice.number, &(voice.ArrayGoToSave));
    restore_2k_array(voice.number, VoiceArrayReadFromFlash, TempArry1);
    pwm_stru.CountDataFromTotally[voice.number] = (uint32_t)((voice.ArrayGoToSave) * (AdcArraySize));
    for (int i = 0; i < AdcArraySize; i++)
    {
      TempArry[i] = TempArry1[i];
    }
  }
  else
  {
...
void ChooseVoiceForPlayfunc()
{
  ...
  voice.number = WitchVoiceWannaToPlay;
...
void RecordStatefunc()
{
  ...
  while (1)
  {
    ...
    {
      conv_adc_val_to_pwm_duty(TempArry); // may be need RTOS
      save_2k_array(voice.number, voice.ArrayGoToSave, TempArry);
      voice.ArrayGoToSave++;
      flag.AdcArryFull = 0;
    }
  ...
  HAL_TIM_Base_Stop_IT(&htim2);
  // voice.number++;
  // WitchVoiceWannaToPlay = voice.number;
  voice.ArrayGoToSave = 0;
  adc_stru.StopTimeCounter = 0;
  HAL_ADC_Stop(&hadc1);
  RecordLED_OFF();
...
void AdcGettingSample()
{
  ...
  {
    // TempArry1[adc_stru.counter] = HAL_ADC_GetValue(&hadc1);
    TempArry1[adc_stru.counter] = ADC_read();
    adc_stru.counter++;
    adc_stru.StopTimeCounter++;
...
void pwm_part()
{
  ...
  {
    __HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_1, (uint16_t)(TempArry[pwm_stru.counter]));
    pwm_stru.counter++;
...
void FlashPrint()
{
  ...
  for (int k = 0; k < 120; k++)
  {
    restore_2k_array(0, a, TempArry1);
    a++;
...
{
  PalyLED_ON();
  SetupForPlay(WitchVoiceWannaToPlay);
  HAL_TIM_Base_Start_IT(&htim2);
