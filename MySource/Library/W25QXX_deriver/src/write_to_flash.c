...
#include "main.h"
#include "w25qxx.h"
#define sector_needs (MaxNumberOfVoice*SampleRate)/AdcArraySize
...
uint8_t save_2k_array(uint8_t count_of_voice, uint8_t count_of_arras, uint16_t *array)
{
    ...
    sector_adrss =((uint32_t) count_of_voice) *((uint32_t) sector_needs) + ((uint32_t)count_of_arras);
...
uint8_t RemoveVoice(uint8_t* which_voic_is_rec, uint8_t voice_numb)
{
    ...
    uint32_t sector_adrss = 0;
    sector_adrss = ((uint32_t)voice_numb) * ((uint32_t)sector_needs);
    for (int i = 0; i < sector_needs; i++)
    {
...
void SaveDetail(uint8_t* which_voic_is_rec, uint8_t voice_numb
 ,uint8_t count_of_array)
{
//    uint32_t pag_addrs_numb_voic = 0;
...
void RestoreDetail(uint8_t* which_voic_is_rec, uint8_t voice_numb
 ,uint8_t* count_of_array)
{
//    uint32_t pag_addrs_numb_voic = 0;
//    pag_addrs_numb_voic =(uint32_t)((detail_sector) * 16);
...
uint8_t HowManyVoiceIsRecord(uint8_t 
 WitchVoiceWannaToPlay_,uint8_t * present_voc_arry)
{
  uint8_t k=0;
  for(uint8_t i=0;i<MaxNumberOfVoice;i++)
  {
